...
export const LogoBluRecruitingFull: React.FC<{ width?: number }> = ({ width = 826 }) => {
  const height = Math.round(width * 131 / 826);
  const sc = height / 1306.5; // scale BLU paths to fit height
  ...
  return (
    <div style={{ filter: "drop-shadow(0 0 10px rgba(0,207,206,0.28))" }}>
      ...
      >
        {/* BLU + teal square — original paths scaled to fit height */}
        ...
        {/* RECRUITING — text aligned to BLU cap height */}
        <text
          x={Math.round(width * 0.256)}
          y={Math.round(height * 0.762)}
          fontFamily="'Helvetica Neue', Helvetica, Arial, sans-serif"
          fontSize={Math.round(height * 0.690)}
          fontWeight="700"
          fill="#ffffff"
